...
const ProductDetailScreen = ({ navigation, route }) => {
  ...
  const handleAddToCart = () => {
    const cartItem = {
      ...
      quantity: quantity,
      addToppings: selectedAddToppings,
      removeIngredients: selectedRemoveToppings,
      specialNotes: specialNotes.trim(),
      totalPrice: calculateTotalPrice(),
      timestamp: new Date().toISOString()
    };

    // Aquí normalmente agregarías al carrito usando Context o Redux
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.imageContainer}>
          <Image
            source={{ uri: product.image_url || 'https://via.placeholder.com/300x200' }}
            ...
            <View style={styles.optionsList}>
              {availableToppings.map((topping) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.optionItem,
                    selectedAddToppings.find(t => t._id === topping._id) && styles.selectedOption
                  ]}
                ...
                >
                  <View style={styles.toppingInfo}>
                    <Text style={[
                      styles.optionText,
                      { color: theme.text },
                      selectedAddToppings.find(t => t._id === topping._id) && { color: theme.primary }
                    ]}>
                      {topping.name}
                    </Text>
                    <Text style={[styles.toppingPrice, { color: theme.textSecondary }]}>
      ...
      <View style={[styles.footer, { backgroundColor: theme.surface, borderTopColor: theme.border }]}>
        <View style={styles.quantitySection}>
          <Text style={[styles.quantityLabel, { color: theme.text }]}>Cantidad</Text>
        ...
        >
          <Text style={styles.addToCartText}>
            Agregar al carrito • {formatPrice(calculateTotalPrice())}
          </Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
